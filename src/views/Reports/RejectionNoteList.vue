<template>
  <section class="container-fluid">
    ...
    <br />
    <div v-if="options.rnotes.length">
            ...
            <b-card-body class="p-2">
              ...
              <b-form-group label="Invoice Type" label-size="sm">
                <b-form-select v-model="filter.type" plain size="sm">
                  ...
                  <b-form-select-option value="rin"
                    >Rejection In</b-form-select-option
                  >
                  <b-form-select-option value="rout"
                    >Rejection Out</b-form-select-option
                  >
                </b-form-select>
              </b-form-group>
      ...
      <report-header>
        <div class="text-center">
          <b>Rejection Note List</b> for the period {{ filter.date.from }} to
          {{ filter.date.to }}
        </div>
      </report-header>
      <b-table
  ...
  methods: {
    ...
    filterData(data) {
      ...
      if (this.filter.type !== 'all') {
        let inout = this.filter.type === 'rin' ? 9 : 15;
        pass = pass && data.inout === inout;
      }
      pass =
      ...
      axios
        ...
        .then((resp) => {
          if (resp.data.gkstatus === 0) {
            self.options.rnotes = resp.data.gkresult.map((memo) => {
