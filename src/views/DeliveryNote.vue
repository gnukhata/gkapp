  ...
  <b-container style="min-width: 300px" fluid class="mt-2 px-md-3 px-2">
    ...
    <b-form @submit.prevent="onSubmit">
      <b-card-group class="d-block d-md-flex my-2" deck>
        ...
        <b-card class="mb-2 mb-md-0 mr-1" border-variant="secondary" no-body>
          <b-overlay
          ...
          <div class="p-2 p-md-3">
            ...
            >
              ...
              <b-form-group
                ...
                label-size="sm"
              >
                <b-form-select v-model="form.delNote.type" size="sm" :options="options.transactionTypes">
                </b-form-select>
              </b-form-group>
              <b-form-group
                :label="isSale ? 'Origin State' : 'Destination State'"
                label-for="input-6"
                label-size="sm"
                label-cols-md="4"
                label-cols="3"
              >
                <autocomplete
                  ...
                  id="input-6"
                  valueUid="id"
                  v-model="form.delNote.state"
                  :options="options.states"
                  @input="setDelNoteGstin(form.delNote.state)"
                  required
                ></autocomplete>
              </b-form-group>
              <b-form-group
                :label="isSale ? 'Dispatch From' : 'Received At'"
                label-for="input-7"
                label-size="sm"
                label-cols-md="4"
                label-cols="3"
        ...
        <!-- Shipping Details -->
        <b-card class="mb-2 mb-md-0 ml-1" border-variant="secondary" no-body>
          <b-overlay :show="isPreloading" variant="secondary" no-wrap blur>
          </b-overlay>
          <div class="p-2 p-md-3">
            <div>
        ...
        <b-table-simple hover small caption-top responsive bordered>
          ...
          <b-tfoot>
            <b-tr variant="secondary" class="text-right">
              ...
              <b-th>
                <span>₹ {{ getTotal('discount', 'amount') }}</span>
              </b-th>
              <b-th>
                <span>₹ {{ getTotal('taxable') }}</span>
              </b-th>
              <b-th colspan="2" v-if="isGst">
                <span>₹ {{ getTotal('igst', 'amount') }}</span></b-th
              >
              <b-th colspan="2" v-if="isGst">
                <span>₹ {{ getTotal('cess', 'amount') }}</span></b-th
              >
              <b-th colspan="2" v-if="!isGst"
                >{>
                <span>{{ getTotal('vat', 'amount') }}</span>
              </b-th>
              <b-th>
      ...
      <b-card-group class="d-block d-md-flex" deck>
            ...
            >
              ...
              >
                <b-form-input
                  size="sm"
                  id="input-24-1"
                  v-model="form.transport.packageCount"
                  type="number"
                  min="0"
                  no-wheel
                  required
        ...
        <b-card class="mb-2 mb-md-0 ml-md-1" border-variant="secondary" no-body>
          <div class="p-2 p-md-3">
            <div>
...
export default {
  ...
  methods: {
    ...
    setDelNoteGstin(state) {
      if (state) {
        this.form.delNote.gstin = this.form.delNote.options.gstin[state.id];
      }
    },
      ...
      return Promise.all([...requests]).then(
        ([resp1, resp2, resp3, resp4, resp5, resp6, resp7]) => {
          ...
          if (resp7.data.gkstatus === 0) {
            self.options.godowns = resp7.data.gkresult.map((godown) => {
              return {
    ...
    fetchUserData() {
      ...
      return axios
        ...
        .then((resp) => {
          ...
          if (resp.status === 200) {
            if (resp.data.gkstatus === 0) {
              self.form.delNote.issuer = resp.data.gkresult.username;
              self.form.delNote.role = resp.data.gkresult.userroleName;
            } else {
      ...
      if (this.options.orgDetails !== null) {
        if (this.options.orgDetails.orgname) {
          ...
          let state = orgstate
            ...
            : null;
          Object.assign(this.form.delNote, {
    ...
    initPayload() {
      let delchal = {
        ...
        roundoffflag: (this.form.totalRoundFlag)? 1 : 0,
        consignee: {},
      ...
      delchal.delchaltotal = total;
      delchal.totalinword = words;
    ...
    onPartyEdit(edited) {
      if (edited) {
        ...
        if (
          this.form.party.pan.length === 10 &&
          this.form.party.checksum.length === 3
        ) {
          payload.gstin = {};
          payload.gstin[
            this.form.party.state.id
          ] = `${this.form.party.state.id}${this.form.party.pan}${this.form.party.checksum}`;
        }
